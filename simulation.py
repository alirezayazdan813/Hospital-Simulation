...
def starting_state():
    ...
    '''
    # Schedule initial power check
    future_event_list.append({
        'event_type': 'Power Outage Check',
        'time': exponential(1 / 480),  # Mean time of 8 hours
        'id': ''
    })'''
...
def emergency_done(state, future_event_list, current_time, patient):
    ...
    if state["emergency_queue"] > 0:
        state["emergency_queue"] -= 1
        # ???????????????????????   patient   ???????????????????????????????????
        print(f"Patient dequeued from emergency queue. Remaining: {state['emergency_queue']}.")

        # Handle lab if necessary
        if state["lab_patients"] < state["lab_capacity"]:
            if state["lab_list"]:
                state["lab_patients"] += 1
                # process patient
...
def pre_surgery_done(state, future_event_list, current_time, patient):
    ...
    if state["pre_surgery_queue"] > 0 and state["pre_surgery_patients"] < state["pre_surgery_capacity"]:
        ...
        print("No patients left in pre-surgery queue.")

    # Update stats or logs
    print(f"Updated state after pre_surgery_done for patient {patient.id}:")
...
def process_emergency(state, future_event_list, current_time, new_patient):
    ...
    if state["emergency_patients"] + 1 <= state["emergency_capacity"]:
